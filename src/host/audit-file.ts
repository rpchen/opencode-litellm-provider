...
export function auditDirectory(environment: NodeJS.ProcessEnv = process.env): string {
...
export async function writeAuditFile(report: object, directory = auditDirectory()): Promise<string> {
  ...
  try {
    ...
    if (!isAbsolute(target)) throw new Error("导出路径不是绝对路径")
    return target
  } finally {
