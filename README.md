...
旧版 `v0.1.0` 不包含模型初始化修复或审查导出：该版本在某些宿主安装路径中，Responses 模型可能可见却因 SDK package 解析失败而无法调用。`v0.1.1` 修复了模型调用，但有已知的 TUI 问题：在已打开的会话中执行导出，文件会写入，结果卡片可能要退出并重新进入同一会话才显示。新版本修复此问题后，请升级并分别通过真实消息及不重进的导出操作验收；不要仅凭模型可见或插件列表判断。
