...
- [ ] 4.1 实现 `/litellm-audit-export` 主动命令、同包 TUI 会话结果卡片与 RPC 反馈；验证完整路径可留存、点击打开和复制、操作失败反馈、迟到 TUI 的最新结果、多会话隔离、不会泄漏到模型请求或影响 provider 注册、poll/reload。
...
- [ ] 5.1 在用户当前连接的真实 OpenCode v2 中手动验证修复构建的命令可见、会话卡片可复制／打开报告、导出文件与当前注册模型集合一致，并分别检查 stale、空清单、连接切换后的报告状态；验证导出本身不会发模型请求且不包含地址或凭据。发布前的已安装 Git package 另行验收。
...
- [ ] 5.4 规划新版本号并在另行获得提交／推送／发布授权后，经新分支 PR 与 required `CI` 合并、按新 tag 发行；分别从默认 GitHub spec 和固定新 tag 安装并验证真实调用和导出，核对 Release 与升级说明。未获发行授权或未完成真实安装验收时，明确保持本任务未完成且不宣称已发布修复。

当前环境的部分验收记录（不代替未勾选的完整任务）：在 OpenCode 2.0.16 原后台服务中将旧 Git 插件条目替换为本地修复构建，确认配置中的活动 provider 为空、插件 `litellm` 已加载且声明 server／TUI 入口。目标 Responses 模型默认和 `#low` variant、已部署 Chat 代表模型的真实消息均返回 `OK`、退出码 0；当前部署没有 Messages 模型。命令在该服务执行成功，RPC 给出文件绝对路径，报告处于 `ready`，18 个导出模型 ID 与宿主当前注册的 18 个 LiteLLM 模型完全一致，没有禁用字段，真实文件和目录只保留当前用户的显式 ACL。用户已在实际 TUI 确认原 toast 可见，并指出无法复制路径或点击打开；新卡片已实现，OpenTUI 测试渲染器中的首次显示、长路径、鼠标点击和失败反馈通过，真实宿主 TUI 的视觉／点击验收仍待完成。顺序门禁（含 v0.1.1 tarball smoke 和干净构建后一致性）已通过；尚未捕获代理端点的直接网络证据，也未改变真实连接来测试 stale／空清单／切换或安装正式 Git 新版本。
