...
export async function openAuditReport(path, run = execFile) {
...
async function pipeToClipboard(command, args, path) {
...
export async function copyAuditPath(path, run = execFile) {
    if (process.platform === "win32") {
        await run("powershell.exe", [
