 ...
 */
export function encodePathForMessage(path) {
    const parts = [];
    for (const character of path) {
        ...
        if (code === 13)
            ...
            parts.push(ESCAPE + "u" + code.toString(16).padStart(4, "0"));
        else
            parts.push(character);
    }
    return parts.join("");
}
export function buildFeedbackMessage(outcome) {
    if (!outcome.ok) {
        return [
 ...
 */
export function createFeedbackSubmitter(session, options = {}) {
    const timeoutMs = options.timeoutMs ?? DEFAULT_TIMEOUT_MS;
    const onError = options.onError ?? (() => { });
    return {
        async submit(sessionID, outcome) {
            let timer;
            try {
